...
Section "Start Menu Shortcuts"
  ...
  CreateShortCut "$SMPROGRAMS\小狼毫输入法\【小狼毫】用户数据同步.lnk" "$INSTDIR\WeaselDeployer.exe" "/sync" "$SYSDIR\shell32.dll" 26
  ...
  CreateShortCut "$SMPROGRAMS\小狼毫输入法\【小狼毫】程序活页夹.lnk" "$INSTDIR\WeaselServer.exe" "/weaseldir" "$SYSDIR\shell32.dll" 19
  ...
  CreateShortCut "$SMPROGRAMS\小狼毫输入法\卸除小狼毫.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
